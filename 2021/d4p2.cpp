...
int main(int argc, char **argv){
    ...
    bool winner = false;
    int lastWinningBoardNo;
    int lastWinningNumber = -1;
    bool assignedLastWinningBoardNo = false;
    dynamic_bitset<> remainingBoards (boards.size()); // boards as a  set of bits
    remainingBoards.set();

    for (int number: numbers) { // for each of the numbers drawn...
        for (int boardNo = 0; boardNo < boards.size() && remainingBoards.test(boardNo); boardNo++) { // for each board that is  left...
            tickNumber(boards, boardNo, number); //tick off the number
            checkSolution(boards, boardNo, number, winner); // and check for a solution

            if (winner) {
                remainingBoards.flip(boardNo);
        ...
        if (remainingBoards.count() == 1 && !assignedLastWinningBoardNo) {
            for (dynamic_bitset<>::size_type i = 0; i < remainingBoards.size(); i++) {
                if (remainingBoards[i] == 1) {
                    lastWinningBoardNo = i;
                    assignedLastWinningBoardNo = true;
                    break;
                }
            }
        }
    ...
    //calculating result
    int result = 0;
    for (cell c: boards[lastWinningBoardNo]) {
        if (!c.ticked) {
            result += c.value;
        }
    }
    printf("board points: %d, lwn: %d", result, lastWinningNumber);

    result *= lastWinningNumber;
