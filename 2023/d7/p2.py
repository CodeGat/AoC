...
def ranking(hand: tuple[str, int]) -> list[int]:
    ...
    jokers: int = result.pop('J', 0)

    j_removed_count: list[int] = sorted(list(result.values()), reverse=True)
    joker_count: list[int] = list(j_removed_count) if len(j_removed_count) != 0 else [0]
    ...
    # if jokers > 0:
    print(f'{hand[0]} w {jokers} J: {count}->{j_removed_count} + {jokers}J->{joker_count}')

    return sorted(joker_count, reverse=True)
