...
def distribute_later_games(current_card_played: int, games_to_go: list[int], amount_of_bonus_cards: int) -> list[int]:
    if amount_of_bonus_cards == 0:
        return games_to_go

    print(f"Adding {games_to_go[current_card_played]} cards to the next {amount_of_bonus_cards} games played after Card {current_card_played + 1}")
    print(games_to_go)
